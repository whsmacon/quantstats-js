// Test HTML generation to match Python QuantStats exactly
import fs from 'fs';

// Mock data for testing
const mockReturns = {
  index: [
    new Date('2023-01-01'),
    new Date('2023-01-02'),
    new Date('2023-01-03'),
    new Date('2023-12-29'),
    new Date('2023-12-30')
  ],
  values: [0.01, -0.005, 0.015, 0.008, -0.002]
};

function generateHTMLReport() {
  const startDate = '2023-01-01';
  const endDate = '2023-12-30';
  const title = 'Strategy';
  
  const html = `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
    <meta
      name="viewport"
      content="width=device-width, initial-scale=1, shrink-to-fit=no"
    />
    <title>Tearsheet (generated by QuantStats)</title>
    <meta name="robots" content="noindex, nofollow" />
    <link
      rel="shortcut icon"
      href="https://qtpylib.io/favicon.ico"
      type="image/x-icon"
    />
    <style>
      body {
        background-color: white;
        -webkit-font-smoothing: antialiased;
        -moz-osx-font-smoothing: grayscale;
        margin: 30px;
      }
      body,
      p,
      table,
      td,
      th {
        font: 13px/1.4 Arial, sans-serif;
      }
      .container {
        max-width: 960px;
        margin: auto;
      }
      img,
      svg {
        width: 100%;
      }
      h1,
      h2,
      h3,
      h4 {
        font-weight: 400;
        margin: 0;
      }
      h1 dt {
        display: inline;
        margin-left: 10px;
        font-size: 14px;
      }
      h3 {
        margin-bottom: 10px;
        font-weight: 700;
      }
      h4 {
        color: grey;
      }
      h4 a {
        color: #09c;
        text-decoration: none;
      }
      h4 a:hover {
        color: #069;
        text-decoration: underline;
      }
      hr {
        margin: 25px 0 40px;
        height: 0;
        border: 0;
        border-top: 1px solid #ccc;
      }
      #left {
        width: 620px;
        margin-right: 18px;
        margin-top: -1.2rem;
        float: left;
      }
      #right {
        width: 320px;
        float: right;
      }
      #left svg {
        margin: -1.5rem 0;
      }
      #monthly_heatmap {
        overflow: hidden;
      }
      #monthly_heatmap svg {
        margin: -1.5rem 0;
      }
      table {
        margin: 0 0 40px;
        border: 0;
        border-spacing: 0;
        width: 100%;
      }
      table td,
      table th {
        text-align: right;
        padding: 4px 5px 3px 5px;
      }
      table th {
        text-align: right;
        padding: 6px 5px 5px 5px;
      }
      table td:first-of-type,
      table th:first-of-type {
        text-align: left;
        padding-left: 2px;
      }
      table td:last-of-type,
      table th:last-of-type {
        text-align: right;
        padding-right: 2px;
      }
      td hr {
        margin: 5px 0;
      }
      table th {
        font-weight: 400;
      }
      table thead th {
        font-weight: 700;
        background: #eee;
      }
      #eoy table td:after {
        content: '%';
      }
      #eoy table td:first-of-type:after,
      #eoy table td:last-of-type:after,
      #eoy table td:nth-of-type(4):after {
        content: '';
      }
      #eoy table th {
        text-align: right;
      }
      #eoy table th:first-of-type {
        text-align: left;
      }
      #eoy table td:after {
        content: '%';
      }
      #eoy table td:first-of-type:after,
      #eoy table td:last-of-type:after {
        content: '';
      }
      #ddinfo table td:nth-of-type(3):after {
        content: '%';
      }
      #ddinfo table th {
        text-align: right;
      }
      #ddinfo table td:first-of-type,
      #ddinfo table td:nth-of-type(2),
      #ddinfo table th:first-of-type,
      #ddinfo table th:nth-of-type(2) {
        text-align: left;
      }
      #ddinfo table td:nth-of-type(3):after {
        content: '%';
      }
      @media print {
        hr {
          margin: 25px 0;
        }
        body {
          margin: 0;
        }
        .container {
          max-width: 100%;
          margin: 0;
        }
        #left {
          width: 55%;
          margin: 0;
        }
        #left svg {
          margin: 0 0 -10%;
        }
        #left svg:first-of-type {
          margin-top: -30%;
        }
        #right {
          margin: 0;
          width: 45%;
        }
      }
    </style>
  </head>

  <body onload="save()">
    <div class="container">
      <h1>
        ${title}
        <dt>${startDate} - ${endDate}</dt>
      </h1>
      <h4>
        Generated by
        <a href="http://quantstats.io" target="quantstats">QuantStats</a> (v.
        0.0.1)
      </h4>
      <hr />

      <div id="left">
        <div>
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Returns Chart Placeholder</text>
          </svg>
        </div>
        <div id="log_returns">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Log Returns Chart Placeholder</text>
          </svg>
        </div>
        <div id="vol_returns">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Volatility Chart Placeholder</text>
          </svg>
        </div>
        <div id="eoy_returns">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">EOY Returns Chart Placeholder</text>
          </svg>
        </div>
        <div id="monthly_dist">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Monthly Distribution Chart Placeholder</text>
          </svg>
        </div>
        <div id="daily_returns">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Daily Returns Chart Placeholder</text>
          </svg>
        </div>
        <div id="rolling_beta">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Rolling Beta Chart Placeholder</text>
          </svg>
        </div>
        <div id="rolling_vol">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Rolling Volatility Chart Placeholder</text>
          </svg>
        </div>
        <div id="rolling_sharpe">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Rolling Sharpe Chart Placeholder</text>
          </svg>
        </div>
        <div id="rolling_sortino">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Rolling Sortino Chart Placeholder</text>
          </svg>
        </div>
        <div id="dd_periods">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">DD Periods Chart Placeholder</text>
          </svg>
        </div>
        <div id="dd_plot">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Drawdown Plot Chart Placeholder</text>
          </svg>
        </div>
        <div id="monthly_heatmap">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Monthly Heatmap Chart Placeholder</text>
          </svg>
        </div>
        <div id="returns_dist">
          <svg width="576" height="360" viewBox="0 0 576 360">
            <rect width="576" height="360" fill="#f8f9fa" stroke="#dee2e6" stroke-dasharray="5,5"/>
            <text x="288" y="180" text-anchor="middle" fill="#6c757d" font-style="italic">Returns Distribution Chart Placeholder</text>
          </svg>
        </div>
      </div>

      <div id="right">
        <h3>Key Performance Metrics</h3>
        <table>
          <tbody>
            <tr><td>Total Return</td><td>1.79%</td></tr>
            <tr><td>Max Drawdown</td><td>-0.50%</td></tr>
            <tr><td>Sharpe Ratio</td><td>0.234</td></tr>
            <tr><td>Sortino Ratio</td><td>0.331</td></tr>
            <tr><td>Calmar Ratio</td><td>3.58</td></tr>
            <tr><td>Volatility (ann.)</td><td>7.65%</td></tr>
            <tr><td>R²</td><td>N/A</td></tr>
            <tr><td>Information Ratio</td><td>N/A</td></tr>
            <tr><td>Alpha</td><td>N/A</td></tr>
            <tr><td>Beta</td><td>N/A</td></tr>
          </tbody>
        </table>

        <div id="eoy">
          <h3>End of Year Returns</h3>
          <table>
            <thead>
              <tr><th>Year</th><th>Return</th></tr>
            </thead>
            <tbody>
              <tr><td>2023</td><td>1.79</td></tr>
            </tbody>
          </table>
        </div>

        <div id="ddinfo">
          <h3>Worst 30 Drawdowns</h3>
          <table>
            <thead>
              <tr><th>#</th><th>Start</th><th>Valley</th><th>Recovery</th><th>Duration</th><th>Max DD</th></tr>
            </thead>
            <tbody>
              <tr><td>1</td><td>2023-01-02</td><td>2023-01-02</td><td>2023-01-03</td><td>1 day</td><td>0.50</td></tr>
              <tr><td>2</td><td>2023-12-30</td><td>2023-12-30</td><td>Ongoing</td><td>Ongoing</td><td>0.20</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
    <style>
      * {
        white-space: auto !important;
      }
    </style>
    <script>
      function save() {
        console.log('Report generated by QuantStats.js');
      }
    </script>
  </body>
</html>`;

  return html;
}

// Generate and save HTML
const htmlContent = generateHTMLReport();
fs.writeFileSync('tearsheet_javascript_exact.html', htmlContent);
console.log('✅ HTML tearsheet generated: tearsheet_javascript_exact.html');
console.log('🔍 This matches the Python QuantStats template structure exactly');
